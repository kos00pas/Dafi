...
class LeaderElectionPhase:
    ...
    def _1_wait_for_non_detached_nodes(self, max_wait=1200, interval=2):
        waited = 0
    ...
    def _2_single_leader_verification(self, max_wait=120, interval=2):
        waited = 0
        while waited <= max_wait:
            self.ns.go(interval)
    ...
    def _3_valid_roles(self, max_wait=120, interval=2):
        waited = 0
        while waited <= max_wait:
            self.ns.go(interval)
            states, _ = self._get_node_states()
    ...
    def _4_rloc16_stability(self, delay=5, max_wait=120, interval=2):
        waited = 0
        while waited <= max_wait:
            def capture():
                return {
    ...
    def _5_ipv6_address_stability(self, delay=5, max_wait=120, interval=2):
        waited = 0
        while waited <= max_wait:
            def get_addrs():
                return {
    ...
    def _6_state_stability(self, delay=5, max_wait=120, interval=2):
        waited = 0
        while waited <= max_wait:
            def capture():
                return {
    ...
    def _7_routing_message_silence(self, delay=5, max_wait=120, interval=2):
        waited = 0
        while waited <= max_wait:
            def get_counters():
                result = {}
